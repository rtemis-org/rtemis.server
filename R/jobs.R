...
# §6.4 (train), §6.6 (cancel), §10 (state machine), §9 (progress).
#
...
#'   (default 8, spec §11.4). Counted across all sessions.
#'
#' @return Job env.
#'
#' @author EDG
...
submit_job <- function(
  ...
  if (!requireNamespace("mirai", quietly = TRUE)) {
    cli::cli_abort(c(
      "Package {.pkg mirai} is required to submit jobs.",
...
#' returns. See spec §6.6 for the honest caveat about compiled code not
