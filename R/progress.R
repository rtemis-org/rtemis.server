...
# Progress channel for rtemislive. See specs/rtemislive.md §9.
#
...
bind_progress_socket <- function(url) {
  if (!requireNamespace("nanonext", quietly = TRUE)) {
    cli::cli_abort(c(
      "Package {.pkg nanonext} is required for the progress channel.",
      "i" = "Install it with {.code install.packages(\"nanonext\")}."
    ))
  }
...
init_daemon_progress <- function(url) {
  if (!requireNamespace("mirai", quietly = TRUE)) {
    cli::cli_abort("Package `mirai` is required.")
  }
  if (!is.character(url) || length(url) != 1L || !nzchar(url)) {
    cli::cli_abort("`url` must be a single non-empty character string.")
  }
